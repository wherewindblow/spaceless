...
void schedule()
{
	context.run_state = SchedulerContext::STARTED;
	...
	context.run_state = SchedulerContext::STOPED;
}
			...
			if (trans)
			{
				switch (trans->trans_type)
				{
					...
					case TransactionType::MULTIPLY_PHASE_TRANSACTION:
					{
						...
						auto result = trans_handler.on_init(conn_id, *package);
		...
		else // Active sleep transaction.
		{
			...
			if (trans_handler)
			{
				...
				if (conn_id == trans_handler->waiting_connection_id() && command == trans_handler->waiting_command())
				{
					...
					//					else
					{
						auto result = trans_handler->on_active(conn_id, *package);
						if (result == MultiplyPhaseTransaction::EXIT_TRANCATION)
						{
...
{
	if (context.run_state == SchedulerContext::STOPED)
	{
		context.run_state = SchedulerContext::STARTING;
		std::thread thread(schedule);
		thread.detach();
	}
	else if (context.run_state == SchedulerContext::STARTED)
	{
...
{
	if (context.run_state == SchedulerContext::STARTED)
	{
		context.stop_flag = true;
		context.run_state = SchedulerContext::STOPING;
	}
}
